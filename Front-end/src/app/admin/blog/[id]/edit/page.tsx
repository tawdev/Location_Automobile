...
import { getAdminBlog, updateAdminBlog, type AdminBlogPayload } from "@/lib/adminBlogsApi";
...
export default function AdminBlogEditPage() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      setError(null);
      try {
        const found = await getAdminBlog(blogId);
        setBlog(found);
        ...
        setError((e as ApiError)?.message || (e instanceof Error ? e.message : t("admin.blogs_load_error")));
      } finally {
        setLoading(false);
      }
    })();
  ...
  async function onSave(payload: AdminBlogPayload) {
    if (!blogId) return;
    setSubmitting(true);
    setError(null);
    try {
      ...
      setError((e as ApiError)?.message || (e instanceof Error ? e.message : t("admin.blog_update_error")));
    } finally {
      setSubmitting(false);
    }
  }
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-3 pt-2">
            ...
            >
              {submitting ? t("admin.saving") : t("admin.save_blog")}
            </button>
            <button
              type="button"
              onClick={() => router.push("/admin/blog")}
