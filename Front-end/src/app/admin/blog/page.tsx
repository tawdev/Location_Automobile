...
import { Plus, Pencil, Trash2, AlertCircle, FileText, MoreHorizontal } from "lucide-react";
    ...
    >
      ...
      <div className="relative flex flex-col items-center max-w-md mx-auto">
        <div className="h-20 w-20 rounded-2xl bg-[#F0F3FA] border border-[#D5DEEF] flex items-center justify-center text-3xl mb-6 shadow-sm">
          <FileText className="w-8 h-8 text-[#638ECB]" />
        </div>
...
export default function AdminBlogPage() {
  ...
  const loadBlogs = useCallback(async () => {
    ...
    try {
      ...
      const msg = (e as ApiError)?.message || (e instanceof Error ? e.message : t("admin.blogs_load_error"));
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  async function onDelete(blogId: number) {
    ...
    try {
      ...
      const msg = (e as ApiError)?.message || (e instanceof Error ? e.message : t("admin.blog_delete_error"));
      setError(msg);
    } finally {
      setDeletingId(null);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <EmptyState onAdd={() => router.push("/admin/blog/new")} />
      ) : (
        <div className="flex flex-col gap-3">
            ...
            {blogs.map((blog) => (
              ...
              >
                <div className="w-12 h-12 rounded-2xl overflow-hidden bg-[#F0F3FA] border border-[#D5DEEF]/40 shrink-0 flex items-center justify-center">
                  <FileText className="w-5 h-5 text-[#638ECB]" />
                </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    ...
                    <span className={`shrink-0 px-2 py-0.5 rounded-full text-[10px] font-bold ${
                      ...
                      {blog.status === "published" ? t("admin.blog_published") : t("admin.blog_draft")}
                    </span>
                    {blog.published_at && (
                ...
                <div className="hidden md:flex items-center gap-2 shrink-0">
                  ...
                  >
                    ...
                    {deletingId === blog.id ? "..." : t("admin.delete")}
                  </button>
                </div>
