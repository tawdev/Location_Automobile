...
import { Plus, Pencil, Trash2, AlertCircle, Briefcase, MoreHorizontal } from "lucide-react";
import type { Career } from "@/lib/types";
import { getAdminCareers, deleteAdminCareer } from "@/lib/adminCareersApi";
    ...
    >
      ...
      <div className="relative flex flex-col items-center max-w-md mx-auto">
        <div className="h-20 w-20 rounded-2xl bg-[#F0F3FA] border border-[#D5DEEF] flex items-center justify-center text-3xl mb-6 shadow-sm">
          <Briefcase className="w-8 h-8 text-[#638ECB]" />
        </div>
...
export default function AdminCareersPage() {
  ...
  const loadCareers = useCallback(async () => {
    ...
    try {
      ...
      const msg = (e as ApiError)?.message || (e instanceof Error ? e.message : t("admin.careers_load_error"));
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  async function onDelete(careerId: number) {
    ...
    try {
      ...
      const msg = (e as ApiError)?.message || (e instanceof Error ? e.message : t("admin.career_delete_error"));
      setError(msg);
    } finally {
      setDeletingId(null);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center gap-3 bg-white rounded-2xl border border-[#D5DEEF]/60 px-5 py-3 shadow-sm">
          <div className="w-10 h-10 rounded-xl bg-[#F0F3FA] flex items-center justify-center">
            <Briefcase className="w-5 h-5 text-[#395886]" />
          </div>
          <div>
      ...
      {loading ? (
        ...
        <EmptyState onAdd={() => router.push("/admin/careers/new")} />
      ) : (
        <div className="flex flex-col gap-3">
            ...
            {careers.map((career) => (
              ...
              >
                <div className="w-12 h-12 rounded-2xl overflow-hidden bg-[#F0F3FA] border border-[#D5DEEF]/40 shrink-0 flex items-center justify-center">
                  <Briefcase className="w-5 h-5 text-[#638ECB]" />
                </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    ...
                    <span className={`shrink-0 px-2 py-0.5 rounded-full text-[10px] font-bold ${
                      ...
                      {career.is_active ? t("admin.career_active") : t("admin.career_inactive")}
                    </span>
                  </div>
                </div>
                ...
                <div className="hidden md:flex items-center gap-2 shrink-0">
                  ...
                  >
                    ...
                    {deletingId === career.id ? "..." : t("admin.delete")}
                  </button>
                </div>
                ...
                <div className="relative md:hidden shrink-0">
                  <button
                    type="button"
                    onClick={() => setOpenMenuId(openMenuId === career.id ? null : career.id)}
