    ...
    >
      ...
      <div className="relative flex flex-col items-center max-w-md mx-auto">
        <div className="h-20 w-20 rounded-2xl bg-[#F0F3FA] border border-[#D5DEEF] flex items-center justify-center text-3xl mb-6 shadow-sm">
          <Building2 className="w-8 h-8 text-[#638ECB]" />
        </div>
...
export default function AdminMarquesPage() {
  ...
  const loadMarques = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await getAdminMarques();
      setMarques(data);
    } catch (e) {
      const msg = (e as ApiError)?.message || (e instanceof Error ? e.message : t("admin.marques_load_error"));
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  async function onDelete(marqueId: number) {
    setDeletingId(marqueId);
    setError(null);
    try {
      await deleteAdminMarque(marqueId);
      await loadMarques();
    } catch (e) {
      const msg = (e as ApiError)?.message || (e instanceof Error ? e.message : t("admin.marque_delete_error"));
      setError(msg);
    } finally {
      setDeletingId(null);
    }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-2xl sm:text-3xl font-black text-[#395886] tracking-tight">
              {t("admin.marques_title")}
            </h1>
          </div>
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => router.push("/admin/marques/bulk")}
              className="inline-flex items-center gap-2 h-11 px-5 rounded-xl border border-[#D5DEEF] text-[#395886] font-extrabold text-sm hover:bg-[#F0F3FA] transition-all active:scale-95 shadow-sm cursor-pointer"
            >
              <Plus className="w-4 h-4" />
            ...
            <button
              type="button"
              onClick={() => router.push("/admin/marques/new")}
              className="inline-flex items-center gap-2 h-11 px-6 rounded-xl bg-[#395886] text-white font-extrabold text-sm hover:bg-[#2D4670] transition-all active:scale-95 shadow-sm cursor-pointer"
            >
              <Plus className="w-4 h-4" />
              {t("admin.add_marque")}
      ...
      >
        <div className="flex items-center gap-3 bg-white rounded-2xl border border-[#D5DEEF]/60 px-5 py-3 shadow-sm">
          <div className="w-10 h-10 rounded-xl bg-[#F0F3FA] flex items-center justify-center">
            <Building2 className="w-5 h-5 text-[#395886]" />
          </div>
          <div>
      ...
      {loading ? (
        ...
        <EmptyState onAdd={() => router.push("/admin/marques/new")} />
      ) : (
        <div className="flex flex-col gap-3">
              ...
              >
                <div className="w-12 h-12 rounded-2xl overflow-hidden bg-[#F0F3FA] border border-[#D5DEEF]/40 shrink-0 flex items-center justify-center">
                  {marque.logo ? (
                    ...
                    <span className="font-black text-lg text-[#638ECB]">
