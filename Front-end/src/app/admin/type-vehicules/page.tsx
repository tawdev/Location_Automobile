...
import { Plus, Pencil, Trash2, AlertCircle, Layers, MoreHorizontal } from "lucide-react";
import type { TypeVehicule } from "@/lib/types";
import { getAdminTypeVehicules, deleteAdminTypeVehicule } from "@/lib/adminTypeVehiculesApi";
    ...
    >
      ...
      <div className="relative flex flex-col items-center max-w-md mx-auto">
        <div className="h-20 w-20 rounded-2xl bg-[#F0F3FA] border border-[#D5DEEF] flex items-center justify-center text-3xl mb-6 shadow-sm">
          <Layers className="w-8 h-8 text-[#638ECB]" />
        </div>
        <h3 className="text-lg font-black text-[#395886] mb-2">
          {t("admin.no_type_vehicules")}
        </h3>
        <button
          type="button"
          onClick={onAdd}
...
export default function AdminTypeVehiculesPage() {
  ...
  const loadTypes = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await getAdminTypeVehicules();
      setTypes(data);
    } catch (e) {
  ...
  async function onDelete(typeId: number) {
    setDeletingId(typeId);
    setError(null);
    try {
      await deleteAdminTypeVehicule(typeId);
      await loadTypes();
    } catch (e) {
      const msg = (e as ApiError)?.message || (e instanceof Error ? e.message : t("admin.type_vehicule_delete_error"));
      setError(msg);
    } finally {
      setDeletingId(null);
    }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-2xl sm:text-3xl font-black text-[#395886] tracking-tight">
              {t("admin.type_vehicules_title")}
            </h1>
          </div>
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => router.push("/admin/type-vehicules/new")}
              className="inline-flex items-center gap-2 h-11 px-6 rounded-xl bg-[#395886] text-white font-extrabold text-sm hover:bg-[#2D4670] transition-all active:scale-95 shadow-sm cursor-pointer"
            >
              <Plus className="w-4 h-4" />
              {t("admin.add_type_vehicule")}
      ...
      >
        <div className="flex items-center gap-3 bg-white rounded-2xl border border-[#D5DEEF]/60 px-5 py-3 shadow-sm">
          <div className="w-10 h-10 rounded-xl bg-[#F0F3FA] flex items-center justify-center">
            <Layers className="w-5 h-5 text-[#395886]" />
          </div>
          <div>
      ...
      {loading ? (
        ...
        <EmptyState onAdd={() => router.push("/admin/type-vehicules/new")} />
      ) : (
        <div className="flex flex-col gap-3">
          <AnimatePresence mode="popLayout">
            {types.map((type) => (
              ...
              >
                <div className="w-10 h-10 rounded-xl bg-[#F0F3FA] border border-[#D5DEEF]/40 shrink-0 flex items-center justify-center">
                  <Layers className="w-5 h-5 text-[#638ECB]" />
                </div>
                ...
                <div className="hidden md:flex items-center gap-2 shrink-0">
                  <button
                    type="button"
                    onClick={() => router.push(`/admin/type-vehicules/${type.id}/edit`)}
                  ...
                  >
                    ...
                    {deletingId === type.id ? "..." : t("admin.delete")}
                  </button>
                </div>
                    ...
                    <>
                      ...
                      <div className="absolute right-0 top-full mt-1 z-20 min-w-[160px] bg-white rounded-2xl border border-[#D5DEEF]/70 shadow-lg py-1.5 overflow-hidden">
                        <button
                          ...
                          onClick={() => {
                            setOpenMenuId(null);
                            router.push(`/admin/type-vehicules/${type.id}/edit`);
                          }}
