...
import { getAdminPressRelease, updateAdminPressRelease, type AdminPressPayload } from "@/lib/adminPressApi";
...
export default function AdminPressEditPage() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      setError(null);
      try {
        const found = await getAdminPressRelease(pressId);
        setPress(found);
        setTitle(found.title);
        setCategory(found.category ?? "");
        setExcerpt(found.excerpt ?? "");
        setContent(found.content ?? "");
        setPublishedAt(found.published_at ?? "");
        setStatus(found.status);
      } catch (e) {
        setError((e as ApiError)?.message || (e instanceof Error ? e.message : t("admin.press_load_error")));
      } finally {
        setLoading(false);
      }
    })();
  ...
  async function onSave(payload: AdminPressPayload) {
    if (!pressId) return;
    setSubmitting(true);
    setError(null);
    try {
      await updateAdminPressRelease(pressId, payload);
      router.push("/admin/press");
    } catch (e) {
      setError((e as ApiError)?.message || (e instanceof Error ? e.message : t("admin.press_update_error")));
    } finally {
      setSubmitting(false);
    }
  }
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-3 pt-2">
            ...
            >
              {submitting ? t("admin.saving") : t("admin.save_press")}
            </button>
            <button
              type="button"
              onClick={() => router.push("/admin/press")}
